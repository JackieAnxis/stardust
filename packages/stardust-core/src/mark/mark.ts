...
const shiftBindingDescriptions = [
  { shift: -2, suffix: "_pp" },
  { shift: -1, suffix: "_p" },
  { shift: +1, suffix: "_n" },
  { shift: +2, suffix: "_nn" }
...
export class Mark {
  ...
  constructor(
    ...
    for (const name in this._spec.input) {
  ...
  public attr(
    name: string,
    value?: BindingValue | ScaleBinding
  ): Mark | BindingValue | ScaleBinding {
    if (value === undefined) {
      ...
      if (value instanceof ScaleBinding) {
        ...
        // Create new binding.
        const newBinding = new Binding(this._spec.input[name].type, value);
        ...
        if (this._platformMark) {
          ...
          if (
            this._platformMark.isUniform(name) &&
            newBinding.bindingType != BindingType.FUNCTION
          ) {
  ...
  public prepareSpecification(): [
    Specification.Mark,
    Dictionary<Binding>,
    Dictionary<ShiftBinding>
  ] {
    const newSpec: Specification.Mark = {
    ...
    const newBindings = this._bindings.clone() as Dictionary<Binding>;
    const shiftBindings = this._shiftBindings.clone();

    this._bindings.forEach((binding, name) => {
      if (binding instanceof ScaleBinding) {
        const attributes = binding.getAttributes();
        const attrs: { [name: string]: Specification.Expression } = {};
        attributes.forEach(attr => {
          const bindedName = name + attr.bindedName;
        ...
        for (const { suffix, shift } of shiftBindingDescriptions) {
          if (newSpec.input.hasOwnProperty(name + suffix)) {
            newSpec.variables[name + suffix] = newSpec.input[name].type;
            const shiftAttrs: { [name: string]: Specification.Expression } = {};
            attributes.forEach(attr => {
              const bindedName = name + attr.bindedName;
              if (
                newBindings.get(bindedName).bindingType == BindingType.FUNCTION
              ) {
                const shiftBindedName = bindedName + suffix;
                shiftBindings.set(
                  shiftBindedName,
                  new ShiftBinding(bindedName, shift)
                );
        ...
        for (const { suffix } of shiftBindingDescriptions) {
      ...
      if (binding instanceof ScaleBinding) {
        ...
        attributes.forEach(attr => {
          if (attr.binding instanceof TextureBinding) {
            this._platformMark.updateTexture(
              name + attr.bindedName,
              attr.binding
            );
          } else {
            this._platformMark.updateUniform(
              name + attr.bindedName,
              attr.binding as Specification.Value
            );
          }
        });
      }
  ...
  public prepare(): Mark {
    if (!this._platformMark) {
      const [spec, binding, shiftBinding] = this.prepareSpecification();
      this._platformMark = this._platform.compile(
        this,
        spec,
        this._shader,
        binding,
    ...
    if (this._shouldUploadData) {
      if (this._instanceFunctions == null) {
        ...
        this._data.forEach((datum, index) => {
          const data = this._instanceFunctions.datum(datum, index, this._data);
          allData.push(data);
        });
    ...
    if (this._instanceFunctions == null) {
      ...
      this._platformMark.render(this._platformMarkData, (index: number) => {
        ...
        if (this._instanceFunctions.attrs) {
          const attrs = this._instanceFunctions.attrs(datum, index, this._data);
          if (attrs != null) {
            for (const attr in attrs) {
              if (attrs.hasOwnProperty(attr)) {
                this._platformMark.updateUniform(
                  attr,
                  getBindingValue(attrs[attr])
                );
              }
            }
          }
